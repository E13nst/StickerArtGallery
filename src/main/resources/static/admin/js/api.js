...
class AdminApiClient {
    ...
    async uploadGlobalStylePresetPreview(id, file) {
        const formData = new FormData();
        formData.append('file', file);

        const response = await fetch(`${this.baseUrl}/generation/style-presets/${id}/preview`, {
            method: 'POST',
            headers: {
                'X-Telegram-Init-Data': this.initData
            },
    ...
    async uploadGlobalStylePresetReference(id, file) {
        const formData = new FormData();
        formData.append('file', file);
        const response = await fetch(`${this.baseUrl}/generation/style-presets/${id}/reference`, {
            method: 'POST',
    ...
    async clearGlobalStylePresetReference(id) {
    ...
    /**
     * Добавляет кадр в галерею доп. превью (основной preview не меняется).
     */
    async appendUserPresetModerationExtraPreview(presetId, file) {
        const formData = new FormData();
        formData.append('file', file);
        const response = await fetch(
            `${this.baseUrl}/admin/style-presets/moderation/${encodeURIComponent(presetId)}/extra-previews`,
            {
                method: 'POST',
                headers: { 'X-Telegram-Init-Data': this.initData },
                body: formData
    ...
    async deleteUserPresetModerationExtraPreview(presetId, cachedImageId) {
        return this.request(
            `/admin/style-presets/moderation/${encodeURIComponent(presetId)}/extra-previews/${encodeURIComponent(cachedImageId)}`,
            { method: 'DELETE' }
        );
    }
